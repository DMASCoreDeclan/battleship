...
def welcome():
    ...
    * denotes a HIT
    ...
    print(
        "This game is designed to allow you to select the grid size you want to play in,\
          ...
          \nAt the end of the game you will be asked if you want to play again or \
...
def draw_board(board, grid_size):
    ...
    for row in board:
        ...
        )  # %s and %d are placeholders for a string and a number respectively. %s returns the string and %d returns a number.  The values are passed using % operator
        grid_row += 1
...
def main(bombs, ships, grid_size):
    while bombs > 0 and count_hit_ships() != ships:
        ...
        print(
            f"You have hit {count_hit_ships()} ships and have {ships - count_hit_ships()} ships left to hit."
        )
        if bombs == 0:
